...
function SumSubject()
{
    ...
    {
        var str  = $.cookie('2').replace('.','');
        var str2 = str.replace(' ','');
        var str3 = parseInt(str2.replace('vnđ',''));
        if($.cookie('2'))
        {
            window.localStorage.setItem('2',str3);
            k.push(str3);
        }
    }

    if($.cookie('12') != undefined){
        var str  = $.cookie('8').replace('.','');
        var str2 = str.replace(' ','');
        var str3 = parseInt(str2.replace('vnđ',''));
        if($.cookie('8'))
        {
            window.localStorage.setItem('8',str3);
            k.push(str3)
        }
    }

    if($.cookie('18') != undefined){
        var str  = $.cookie('14').replace('.','');
        var str2 = str.replace(' ','');
        var str3 = parseInt(str2.replace('vnđ',''));
        if($.cookie('14'))
        {
            window.localStorage.setItem('14',str3);
            k.push(str3);
        }
    }

    if(k.length > 0) {
        var sum = k.reduce(function(a, b) { return a + b; }, 0);
        var iTotal  = sum + 10000;
        var total = FormatMoney(iTotal) + ' VNĐ';
        //jQuery('#TotalMoney').html(total);
    }
    return k.length;
}
...
(function ($) {
  ...
  $(document).ready(function () {
    ...
    if($.cookie('6') != undefined)
    {
      ...
      $('#bill-6-khoa').html($.cookie('1'));
      $('#bill-6-phongkham').html($.cookie('5'));
      $('#bill-6-giokham').html($.cookie('4'));
      var buoi  = '<i>'+$.cookie('6')+'</i>';
      $('#bill-6-buoi').html(buoi);
      ...
      $showHtml += ' <td><button data-id="2">Xóa</button></td>';
      $showHtml += '</tr>';
    ...
    if($.cookie('12') != undefined){
      ...
      $('.booking-left-menu-ordered p').eq(3).find('span').text($.cookie('10'));
        ...
        $('#bill-12-khoa').html($.cookie('7'));
        $('#bill-12-phongkham').html($.cookie('11'));
        $('#bill-12-giokham').html($.cookie('10'));
        var buoi  = '<i>'+$.cookie('12')+'</i>';
        $('#bill-12-buoi').html(buoi);
      ...
      $showHtml += ' <td><button data-id="8">Xóa</button></td>';
      $showHtml += '</tr>';
      $('.booking-right-info .bri-show-records tbody').append($showHtml);
    ...
    if($.cookie('18') != undefined){
      ...
      $('.booking-left-menu-ordered p').eq(3).find('span').text($.cookie('16'));
        ...
        $('#bill-12-khoa').html($.cookie('13'));
        $('#bill-12-phongkham').html($.cookie('17'));
        $('#bill-12-giokham').html($.cookie('16'));
        var buoi  = '<i>'+$.cookie('18')+'</i>';
        $('#bill-12-buoi').html(buoi);
      ...
      $showHtml += ' <td><button data-id="14">Xóa</button></td>';
      $showHtml += '</tr>';
      $('.booking-right-info .bri-show-records tbody').append($showHtml);
